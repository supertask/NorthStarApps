Shader "SineVFX/LivingParticles/LivingParticleFloorPbrArray"
{
	Properties
	{
		...
		[Toggle]_XYUV("XY UV", Int) = 0
		[Toggle]_ZYUV("ZY UV", Int) = 0
	...
	SubShader
	{
		...
		void vertexDataFunc( inout appdata_full v, out Input o )
		{
			...
			float2 appendResult172 = (float2(appendResult17.z , appendResult17.y));
			float2 appendResult171 = (float2(appendResult17.x , appendResult17.y));
			float2 appendResult104 = (float2(appendResult17.x , appendResult17.z));
			#ifdef _XYUV_ON
				float2 staticSwitch169 = appendResult171;
			#else
				float2 staticSwitch169 = appendResult104;
			#endif
			#ifdef _ZYUV_ON
				float2 staticSwitch170 = appendResult172;
			#else
				float2 staticSwitch170 = staticSwitch169;
			#endif
			float2 ParticlePositionUV106 = staticSwitch170;
			...
			float4 appendResult165 = (float4(v.texcoord1.y , v.texcoord1.z , v.texcoord1.w , 0.0));
			float4 normalizeResult164 = normalize( appendResult165 );
			v.vertex.xyz += ( ( 1.0 - ResultMaskModified90 ) * _OffsetPower * normalizeResult164 ).xyz;
		}
